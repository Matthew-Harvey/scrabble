...
class main:
    ...
    def windowcreate(self, allinputs):
        ...
        del self.user[-1]
        window=tk.Tk()
        tk.Label(window, text=self.string).grid(row=2)
        main.scoring()
        self.score = "Your score is "+ str(self.score)
        tk.Label(window, text=" ").grid(row=3, column=1)
        tk.Label(window, text=self.score).grid(row=4)
        tk.Label(window, text=self.wordlist).grid(row=4, column=1)
